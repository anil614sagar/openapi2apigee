...
module.exports = function generatePolicies(apiProxy, options, api, cb) {
  ...
  async.each(Object.keys(api['x-a127-services']), function(service, callback) {
    ...
    if (provider.indexOf('cors') > -1) {
      // Add cors Policies
      xmlString = cors.corsGenTemplate(api['x-a127-services'][service].options, service);
    }
    if (provider.indexOf('headers') > -1) {
      // Add header Policies
      xmlString = headers.headersGenTemplate(api['x-a127-services'][service].options, service);
    }
    if (provider.indexOf('regex') > -1) {
      // Add regex Policies
      xmlString = regex.regexGenTemplate(api['x-a127-services'][service].options, service);
      // filter
    ...
    if (provider.indexOf('raiseFault') > -1) {
      ...
      xmlString = raiseFault.raiseFaultGenTemplate(api['x-a127-services'][service].options, service);
    }
    if (provider.indexOf('input-validation') > -1) {
      xmlString = validations.validationsGenTemplate(api['x-a127-services'][service].options, service);
    }
    ...
    if (provider.indexOf('input-validation') > -1 || provider.indexOf('output-validation') > -1) {
      ...
      var x = validations.validationsSchemas();
      var wstream = fs.createWriteStream(rootDirectory + '/resources/jsc/schemas.js');
      wstream.write(new Buffer('var elements = ' + JSON.stringify(x) + ';'));
      wstream.end();
    }

    if (provider.indexOf('raiseValidationFault') > -1) {
      // Add RaiseFault Policy
      xmlString = raiseFault.raiseFaultGenTemplate(api['x-a127-services'][service].options, service);
    }
