...
module.exports = function generatePolicies(apiProxy, options, api, cb) {
  ...
  async.each(Object.keys(api['x-a127-services']), function(service, callback) {
    ...
    if (provider.indexOf('regex') > -1) {
      ...
      // streamFromArray(x).pipe(fs.createWriteStream(rootDirectory + '/resources/jsc/'+ 'regex' + '.js'));
      // fs.createReadStream(js).pipe(fs.createWriteStream(rootDirectory + '/resources/jsc/'+ 'regex' + '.js'));
