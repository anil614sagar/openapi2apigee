  ...
  for (var apiPath in api.paths) {
    for (var resource in api.paths[apiPath]) {
      if (allowedVerbs.indexOf(resource.toUpperCase()) >= 0) {
        ...
        if (raiseFaultName && apiPath === '/animals/cats' && resource.toUpperCase() === 'GET') {
          var requestStep = requestPipe.ele('Step');
          requestStep.ele('Condition', {}, '(request.queryparam.postalCode Equals null) or (request.queryparam.houseNumber Equals null)');
