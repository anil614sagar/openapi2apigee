      ...
      if (allowedVerbs.indexOf(resource.toUpperCase()) >= 0) {
        ...
        if (resourceItem['x-a127-apply']) {
          for (var service in resourceItem['x-a127-apply']) {
            if(resourceItem['x-a127-apply'][service].endPoint == "proxy") {
              if (resourceItem['x-a127-apply'][service].pipe == "request") {
                ...
                step.ele("Name", {}, service);
              } // pipe request / response if else ends here
            }  // proxy check ends here
          } // for loop ends here
        } // check for policies ends here
