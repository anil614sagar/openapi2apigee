  ...
  elements.forEach(function (element) {
    ...
    element.elements.forEach( function (e) {
      ...
      if (data) {
        ...
        data.filters.push(element.rule.toString());
      } else {
        //console.log('add', e.element);
        elementsArray.push({ element: e.element, name: e.name, filters: [element.rule.toString()]});
      }
    })
  });
...
function regularExpressions() {
  ...
  var l = path.join('./');
  console.log('l', l);
  var regex = require('../../../regex_rules.json');
  return regex;
}
